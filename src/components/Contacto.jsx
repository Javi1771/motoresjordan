...
import { Mail, MapPin, Clock, Send, Users, Award, Shield, Headphones } from 'lucide-react';
...
export default function Contacto() {
  ...
  const handleWhatsApp = () => {
    ...
    setTimeout(() => {
      const message = encodeURIComponent(`Hola, solicito cotización para equipos de bombeo.
...
Gracias por su atención.`);
  ...
  const handleEmailContact = () => {
    ...
    setIsSubmitting(true);
    setTimeout(() => {
      const subject = encodeURIComponent(`Solicitud de Cotización - ${formData.name}`);
      const body = encodeURIComponent(`Estimado equipo de Motores Jordan,
...
${formData.name}`);
      ...
      window.open(`mailto:contacto@motoresjordanmx.com?subject=${subject}&body=${body}`, '_blank');
      setIsSubmitting(false);
    }, 1000);
  };
  ...
  return (
    <section id="contact" className="relative bg-gradient-to-br from-[#000000] via-[#0a0a0a] to-[#1a1a1a] overflow-hidden">
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-12 lg:py-20 relative">
        {/* Header Section */}
        <div className={`text-center mb-12 lg:mb-16 transition-all duration-1000 ${isVisible ? 'translate-y-0 opacity-100' : 'translate-y-8 opacity-0'}`}>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 lg:gap-6 mt-12 lg:mt-16 max-w-4xl mx-auto">
            {stats.map((stat, index) => (
              <div 
                key={index}
                className={`bg-gradient-to-br from-[#262626]/90 to-[#1a1a1a]/90 backdrop-blur-sm rounded-xl lg:rounded-2xl p-4 lg:p-6 border border-[#4E4F50]/50 hover:border-[#BE171F]/50 transition-all duration-500 hover:-translate-y-1 hover:shadow-xl hover:shadow-[#BE171F]/10 ${isVisible ? 'translate-y-0 opacity-100' : 'translate-y-8 opacity-0'}`}
              ...
              >
                ...
                <div className="text-xs sm:text-sm text-[#8E8F91] font-medium">{stat.label}</div>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-6 lg:gap-8 xl:gap-12 max-w-7xl mx-auto">
          {/* Contact Info */}
          <div className={`space-y-6 lg:space-y-8 transition-all duration-1000 delay-300 ${isVisible ? 'translate-x-0 opacity-100' : '-translate-x-8 opacity-0'}`}>
            ...
            <div className="bg-gradient-to-br from-[#262626]/90 to-[#1a1a1a]/90 backdrop-blur-sm rounded-xl lg:rounded-2xl p-6 lg:p-8 border border-[#4E4F50]/50 hover:border-[#BE171F]/50 transition-all duration-300 hover:shadow-xl hover:shadow-[#BE171F]/10">
              <div className="flex items-center mb-6 lg:mb-8">
                ...
                <h3 className="text-xl lg:text-2xl font-bold text-white">Información de Contacto</h3>
              </div>
              ...
              <div className="space-y-6">
                <div className="flex items-start space-x-4">
                  ...
                  <div>
                    ...
                    <p className="text-[#8E8F91] leading-relaxed">
                      C. José María Morelos 126-C, Centro<br />
                ...
                <div className="flex items-start space-x-4">
                  ...
                  <div>
                    ...
                    <p className="text-[#8E8F91] break-all sm:break-normal">contacto@motoresjordanmx.com</p>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="bg-gradient-to-br from-[#262626]/90 to-[#1a1a1a]/90 backdrop-blur-sm rounded-xl lg:rounded-2xl p-6 lg:p-8 border border-[#4E4F50]/50 hover:border-[#BE171F]/50 transition-all duration-300 hover:shadow-xl hover:shadow-[#BE171F]/10">
              <div className="flex items-center mb-6 lg:mb-8">
                ...
                <h3 className="text-xl lg:text-2xl font-bold text-white">Horario de Atención</h3>
              </div>
              
              <div className="space-y-3">
                <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center py-3 px-4 bg-[#1a1a1a]/50 rounded-lg">
                  <span className="text-[#8E8F91] font-medium">Lunes - Viernes</span>
          ...
          <div className={`transition-all duration-1000 delay-500 ${isVisible ? 'translate-x-0 opacity-100' : 'translate-x-8 opacity-0'}`}>
            <div className="bg-gradient-to-br from-[#262626]/90 to-[#1a1a1a]/90 backdrop-blur-sm rounded-xl lg:rounded-2xl p-6 lg:p-8 border border-[#4E4F50]/50 hover:border-[#BE171F]/50 transition-all duration-300 hover:shadow-xl hover:shadow-[#BE171F]/10">
              <div className="flex items-center mb-6 lg:mb-8">
                ...
                <h3 className="text-xl lg:text-2xl font-bold text-white">Solicita una Cotización</h3>
              </div>
              
              <form className="space-y-6" onSubmit={(e) => e.preventDefault()}>
                <div>
                  <label className="block text-[#D0D0D1] font-medium mb-2">Nombre *</label>
                  <input
                    type="text"
                    name="name"
                    value={formData.name}
                    onChange={handleInputChange}
                    className={`w-full px-4 py-3 bg-[#1a1a1a]/80 border rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-[#BE171F] focus:border-transparent transition-all duration-200 ${formErrors.name ? 'border-red-500' : 'border-[#6B6C6F]'}`}
                ...
                <div>
                  <label className="block text-[#D0D0D1] font-medium mb-2">Email *</label>
                  <input
                    type="email"
                    name="email"
                    value={formData.email}
                    onChange={handleInputChange}
                    className={`w-full px-4 py-3 bg-[#1a1a1a]/80 border rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-[#BE171F] focus:border-transparent transition-all duration-200 ${formErrors.email ? 'border-red-500' : 'border-[#6B6C6F]'}`}
                ...
                <div>
                  <label className="block text-[#D0D0D1] font-medium mb-2">Teléfono *</label>
                  <input
                    type="tel"
                    name="phone"
                    value={formData.phone}
                    onChange={handleInputChange}
                    className={`w-full px-4 py-3 bg-[#1a1a1a]/80 border rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-[#BE171F] focus:border-transparent transition-all duration-200 ${formErrors.phone ? 'border-red-500' : 'border-[#6B6C6F]'}`}
                ...
                <div>
                  <label className="block text-[#D0D0D1] font-medium mb-2">Mensaje *</label>
                  <textarea
                    rows={4}
                    name="message"
                    value={formData.message}
                    onChange={handleInputChange}
                    className={`w-full px-4 py-3 bg-[#1a1a1a]/80 border rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-[#BE171F] focus:border-transparent transition-all duration-200 resize-none ${formErrors.message ? 'border-red-500' : 'border-[#6B6C6F]'}`}
